...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [EventCenter Event_Add:MSG_KLINE_SUCCESS obj:self];
}
...
-(void)handlerEvent:(NSNotification *)note
{
    if( note.object != nil)
    {
        ...
        switch (event.what)
        {
            case MSG_KLINE_SUCCESS:
            {
                XResponse *response = event.obj;
//                response.raw_Json
                NSLog(@"%@",response.raw_Json);
                KLineItem *klineItem = [KLineItem yy_modelWithDictionary:response.raw_Json];
...
- (void)setupChart{
    ...
    dismissBtn.frame = CGRectMake(SCREEN_MAX_LENGTH-40, 10, 30, 30);
    dismissBtn.backgroundColor = [UIColor lightGrayColor];
